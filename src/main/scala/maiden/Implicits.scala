...
object DateImplicits {
  ...
  implicit val decodeLocalTime = mappedEncoding[Date, LocalDateTime](date =>
    LocalDateTime.ofInstant(date.toInstant, ZoneId.systemDefault()))

  implicit val encodeLocalTime = mappedEncoding[LocalDateTime, Date](time =>
    Date.from(time.atZone(ZoneId.systemDefault).toInstant))

  //handle ordering of datetimes
  implicit val timestampOrder: Ordering[LocalDateTime] = null
